...
def unloadAttributes(attributes={}):
    meta = list(filter(lambda x: x[1] != 0 or x[0] == "alertid", [(attributes[x]['xml_parser'], attributes[x]['c'], funcs[attributes[x]['h']['options'][attributes[x]['h']['chosen']]])
                                                                  for x in attributes if attributes[x]['use']]))
    if meta[0][0] != "alertid":
...
def unloadDisplay(display={}):
    ...
    if display.get('layout') and display['layout'].get('chosen') and display['layout'].get('options'):
    ...
    if display.get('arrow_style') and display['arrow_style'].get('options') and display['arrow_style'].get('chosen'):
    ...
    if display.get('node_shape') and display['node_shape'].get('options') and display['node_shape'].get('chosen'):
